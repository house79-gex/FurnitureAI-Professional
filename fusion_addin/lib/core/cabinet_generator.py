...
# Costante per conversione unità: Fusion 360 usa cm internamente
MM_TO_CM = 10.0
...
class CabinetGenerator:
    ...
    def create_cabinet(self, params):
        ...
        if has_back:
            ...
            # Configura parametri montaggio retro
            back_config = {
                'mounting': back_mounting,
                'rabbet_width': rabbet_width,
                'rabbet_depth': rabbet_depth,
                'groove_width': groove_width,
                'groove_depth': groove_depth,
                'groove_offset': groove_offset_from_rear,
                'retro_inset': retro_inset
            }
            
            self._create_back_panel(cabinet_comp, width, height, depth, thickness, back_thickness, 
                                   has_plinth, plinth_height, back_config)
    ...
    def _create_side_panels(self, component, width, height, depth, thickness, has_plinth, plinth_height):
        ...
        z_start = plinth_height / MM_TO_CM if has_plinth else 0
    ...
    def _create_top_bottom_panels(self, component, width, depth, thickness, height=None, has_plinth=False, plinth_height=0):
        ...
        rect_bottom = sketch_bottom.sketchCurves.sketchLines.addTwoPointRectangle(
            adsk.core.Point3D.create(0, z_bottom / MM_TO_CM, 0),
        ...
        distance_bottom = adsk.core.ValueInput.createByReal(W_in / MM_TO_CM)
        ...
        if height is not None:
            ...
            rect_top = sketch_top.sketchCurves.sketchLines.addTwoPointRectangle(
                adsk.core.Point3D.create(0, z_top / MM_TO_CM, 0),
            ...
            distance_top = adsk.core.ValueInput.createByReal(W_in / MM_TO_CM)
    ...
    def _create_back_panel(self, component, width, height, depth, thickness, back_thickness, 
                          has_plinth, plinth_height, back_config):
        """
        ...
        Args:
            back_config: Dizionario con configurazione montaggio retro
                - mounting: Tipo montaggio ('flush_rabbet', 'groove', 'surface')
                ...
                - groove_offset: Offset canale dal retro (mm)
        ...
        back_mounting = back_config.get('mounting', 'flush_rabbet')
        rabbet_width = back_config.get('rabbet_width', 12)
        rabbet_depth = back_config.get('rabbet_depth', back_thickness)
        groove_width = back_config.get('groove_width', back_thickness + 0.5)
        groove_depth = back_config.get('groove_depth', back_thickness)
        groove_offset_from_rear = back_config.get('groove_offset', 10)
        retro_inset = back_config.get('retro_inset', 0)
        ...
        rect = sketch.sketchCurves.sketchLines.addTwoPointRectangle(
            adsk.core.Point3D.create(y_position / MM_TO_CM, z_offset / MM_TO_CM, 0),
            adsk.core.Point3D.create((y_position + back_thickness) / MM_TO_CM, (z_offset + panel_height) / MM_TO_CM, 0)
        )
    ...
    def _create_shelves(self, component, width, depth, thickness, height, count, has_plinth, plinth_height,
        ...
        for i in range(count):
            ...
            sketch = sketches.add(yz_plane)
            rect = sketch.sketchCurves.sketchLines.addTwoPointRectangle(
                adsk.core.Point3D.create(shelf_front_setback / MM_TO_CM, z_position / MM_TO_CM, 0),
                adsk.core.Point3D.create((shelf_front_setback + shelf_depth_eff) / MM_TO_CM, (z_position + thickness) / MM_TO_CM, 0)
            )
            ...
            distance = adsk.core.ValueInput.createByReal(W_in / MM_TO_CM)
            extrude_input.setDistanceExtent(False, distance)
    ...
    def _create_divisions(self, component, width, height, depth, thickness, count, has_plinth, plinth_height):
        ...
        # Calculate Z offset for dividers
