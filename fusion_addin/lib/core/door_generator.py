...
- v3.1: Posizionamento parametrico SENZA dipendenza da bounding box
...
- Allineamento finale: usa PARAMETRI per posizionamento preciso
...
MODIFICHE v3.1:
- Posizionamento ante basato su PARAMETRI (depth, plinth_height, x_offset)
- Bounding box usato SOLO per debug/verifica, non per logica
- Eliminata dipendenza da coordinate bbox della carcassa
"""
...
class DoorGenerator:
    ...
    def create_door(self, params):
        ...
        POSIZIONAMENTO v3.1 (PARAMETRICO):
        - X: x_offset + side_gap
        - Y: plinth_height + bottom_gap
        - Z: depth (fronte cabinet)
        ...
        self.logger.info(f"   Cabinet depth: {cabinet_depth}mm")
        ...
        # --- POSIZIONAMENTO PARAMETRICO v3.1 (SENZA BOUNDING BOX) ---
        try:
            if door_comp.bRepBodies.count == 0:
                self.logger.warning("   ⚠️ Nessun body creato nell'anta, salto posizionamento")
                return door_comp
            
            door_body = door_comp.bRepBodies.item(0)
            door_bbox_local = door_body.boundingBox
            
            self.logger.info(f"   📦 Bbox anta locale (prima posizionamento):")
            ...
            # ✅ POSIZIONAMENTO PARAMETRICO (v3.1)
            # Calcola posizione finale usando SOLO parametri, non bbox cabinet
            
            # 1. X: offset laterale + gap
            desired_x_min_cm = (x_offset_mm + side_gap_mm) / 10.0
            delta_x = desired_x_min_cm - door_bbox_local.minPoint.x
            
            # 2. Y: base anta al top dello zoccolo + gap inferiore
            desired_y_min_cm = (cabinet_plinth_height + bottom_gap_mm) / 10.0
            delta_y = desired_y_min_cm - door_bbox_local.minPoint.y
            
            # 3. Z: faccia interna anta al fronte del cabinet
            #    (faccia interna = Z_min locale, deve andare a Z = cabinet_depth)
            desired_z_min_cm = cabinet_depth / 10.0  # fronte cabinet in cm
            delta_z = desired_z_min_cm - door_bbox_local.minPoint.z
            
            self.logger.info(f"   🔧 Delta posizionamento parametrico (v3.1):")
            ...
            self.logger.info(f"   ✅ Anta posizionata parametricamente (v3.1)")
            ...
            # DEBUG: MessageBox opzionale
            try:
                app = adsk.core.Application.get()
                ui = app.userInterface
                ui.messageBox(
                    f"DEBUG ANTA BBOX (v3.1 PARAMETRICO):\n"
                    f"x=({door_bbox_final.minPoint.x:.2f}, {door_bbox_final.maxPoint.x:.2f}) cm\n"
                    f"y=({door_bbox_final.minPoint.y:.2f}, {door_bbox_final.maxPoint.y:.2f}) cm\n"
                    f"z=({door_bbox_final.minPoint.z:.2f}, {door_bbox_final.maxPoint.z:.2f}) cm"
                )
            except:
                pass
            
            # Verifica opzionale con bbox cabinet (solo per debug)
            if parent_component and parent_component.bRepBodies.count > 0:
                for body in parent_component.bRepBodies:
                    if "Fianco_Sinistro" in body.name or "Fianco_Sinistra" in body.name:
                        ...
                        self.logger.info(f"   📦 Bbox cabinet (fianco sinistro) per verifica:")
            ...
            self.logger.error(f"   ❌ Errore posizionamento anta: {e}")
            import traceback
    ...
    def _create_flat_door(self, component, width, height, thickness):
        ...
        Geometria locale: XY plane (Z=0), estrusione in +Z per thickness
        """
        ...
        # Arrotondamento spigoli (opzionale)
        try:
